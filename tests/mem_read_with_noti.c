...
//   }  
  ...
  buffer_size=bytes;
  setup_buffer(port, &send_buffer, &send_nla_buffer_region, buffer_size,0xb30);
  ...
  while (bytes > 0 )
  {
    ...
    //bytes_to_send= (bytes > 120) ? 120 : bytes;
    bytes=bytes - bytes_to_send;
    if (cmd_type==0)
      rc=rma2_post_get_qw(port, handle, recv_nla_buffer_region, recv_offset, bytes_to_send, send_nla,RMA2_RESPONDER_NOTIFICATION | RMA2_COMPLETER_NOTIFICATION|RMA2_REQUESTER_NOTIFICATION,RMA2_CMD_DEFAULT);
    else if (cmd_type==1)
      rc=rma2_post_get_bt(port, handle, recv_nla_buffer_region, recv_offset, bytes_to_send, send_nla,RMA2_RESPONDER_NOTIFICATION | RMA2_COMPLETER_NOTIFICATION|RMA2_REQUESTER_NOTIFICATION,RMA2_CMD_DEFAULT);
    else
    {
    ...
    recv_offset=recv_offset+bytes_to_send;
    send_nla=send_nla+bytes_to_send;
  }
  //sleep(1);
  noti_number=3 * requests;
  ...
  for (i=0;i<upper_boarder;i++)
  {
    ...
    {
      DEBUG(printf("Mismatch at %d: %lx %lx\n",i,send_buffer[i],recv_buffer[i]));
    }
  }
