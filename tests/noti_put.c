...
//comment if you want to enable translations
//#define PHYS_NOTI
...
int main(int argc, char** argv)
{
  ...
  if (argc<5)
  {
    fprintf(stderr,"Erroneous commandline. Must provide 4 parameter: destination node, destination vpid and value and count\n");//, registerfile address, value and command typeto write into register\n");
    //fprintf(stderr," Command type can be 0 - put qw, 1 - imm put or 2 - put bt\n");
    abort();
  }
  dest_nodeid=strtol(argv[1],0,0);
  dest_vpid=strtol(argv[2],0,0);
  value=strtol(argv[3],0,0);
  count=strtol(argv[4],0,0);
  ...
  printf("%ld notifications per put...\n",(unsigned long int)noti_number);
  for (i=0;i<count;i++)
  {
    rc=rma2_post_notification(port, handle,42+i,value, RMA2_ALL_NOTIFICATIONS,0);
    if (rc!=RMA2_SUCCESS)
    {
