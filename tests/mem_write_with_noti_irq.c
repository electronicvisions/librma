...
int main(int argc, char** argv)
{
  ...
  conn_type= RMA2_CONN_DEFAULT| RMA2_CONN_IRQ;    
  ...
  for (trans=0; trans<2; trans++)
  {
	...
	rc=rma2_post_put_qw(port, handle, send_nla_buffer_region, send_offset, bytes_to_send, recv_nla,RMA2_ALL_NOTIFICATIONS,0);
      else if (cmd_type==1)
      {
	rc=rma2_post_put_bt(port, handle, send_nla_buffer_region, send_offset, bytes_to_send, recv_nla,RMA2_ALL_NOTIFICATIONS,0);
      }
      else
      {
