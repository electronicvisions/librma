...
int main(int argc, char** argv)
{
  ...
  if (argc<5)
  {
    fprintf(stderr,"Erroneous commandline. Must provide 3 parameter: destination node, registerfile address, value and command type to write into register\n");
    fprintf(stderr," Command type can be 0 - put qw, 1 - put bt or 2 - imm put\n");
    abort();
  }
  ...
  buffer[0]=value;
  if (cmd_type==0)
    rc=rma2_post_put_qw_direct(port, handle, p_buffer, 8, dest_rra_address,RMA2_REQUESTER_NOTIFICATION,RMA2_CMD_DEFAULT);
  else if (cmd_type==1)
    rc=rma2_post_put_bt_direct(port, handle, p_buffer, 8, dest_rra_address,RMA2_REQUESTER_NOTIFICATION,RMA2_CMD_DEFAULT);
  else if (cmd_type==2)
    rc=rma2_post_immediate_put(port, handle, 8, value,dest_rra_address , RMA2_REQUESTER_NOTIFICATION,RMA2_CMD_DEFAULT);
  else
  {
