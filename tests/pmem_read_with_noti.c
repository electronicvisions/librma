...
int main(int argc, char** argv)
{
  ...
  if (argc<6)
  {
    fprintf(stderr,"Erroneous commandline. Must provide 5 parameter: command type and number of bytes to write, remote node, remote vpid and remote physical address.\n");
    fprintf(stderr," Command type can be 0 - get qw, 1 - get bt\n");
    fprintf(stderr,"give a physical address of zero if loopbacking to self (program will fill in physical address automatically)\n");
    abort();
  }
  //dest_nodeid=strtol(argv[1],0,0);
  cmd_type=strtol(argv[1],0,0);
  bytes=strtol(argv[2],0,0);
  dest_nodeid=strtol(argv[3],0,0);
  dest_vpid=strtol(argv[4],0,0);
  dest_address=strtol(argv[5],0,0);
 ...
 // printf("Accessing register at address 0x%lx on node %d: writing %lx\n",dest_rra_address,dest_nodeid,value);
   ...
   if (nodeid==dest_nodeid && dest_vpid==vpid) //loopback to self
   {
    ...
    dest_address=send_p_buffer;
    }
  else
    noti_number=2;
  
  if (cmd_type==0)
    rc=rma2_post_get_qw_direct(port, handle, recv_p_buffer, bytes, dest_address,RMA2_ALL_NOTIFICATIONS,0);
  else if (cmd_type==1)
    rc=rma2_post_get_bt_direct(port, handle, recv_p_buffer, bytes, dest_address,RMA2_ALL_NOTIFICATIONS,0);
  else
  {
 ...
 for (i=0;i<noti_number;i++)
  ...
  if (nodeid==dest_nodeid && dest_vpid==vpid) //loopback to self -> check data!
  {
