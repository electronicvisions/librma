...
int main(int argc, char** argv)
{
  ...
  if (cmd_type==0)
    ...
    rc=rma2_post_put_qw_direct(port, handle, send_p_buffer, bytes, recv_p_buffer,0,0);
  else if (cmd_type==1)
    rc=rma2_post_put_bt_direct(port, handle, send_p_buffer, bytes, recv_p_buffer,0,0);
