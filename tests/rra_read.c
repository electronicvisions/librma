...
int main(int argc, char** argv)
{
  ...
  if (cmd_type==0)
    rc=rma2_post_get_qw_direct(port, handle, p_buffer, 8,dest_rra_address, RMA2_COMPLETER_NOTIFICATION,RMA2_CMD_DEFAULT);
  else if (cmd_type==1)
    rc=rma2_post_get_bt_direct(port, handle, p_buffer, 8, dest_rra_address,RMA2_COMPLETER_NOTIFICATION,RMA2_CMD_DEFAULT);
  else
  {
